class DropBoxController {
    ...
    constructor(){
        ...
        this.currentFolder = ['Meus arquivos'];
    ...
    connectFirebase(){
        ...
        var firebaseConfig = {
            apiKey: "AIzaSyABgvN0RU-mTPVCK_DIiO8ZvPkcB-HQDAE",
            authDomain: "dropbox-clone-991c0.firebaseapp.com",
            databaseURL: "https://dropbox-clone-991c0.firebaseio.com",
            projectId: "dropbox-clone-991c0",
            storageBucket: "gs://dropbox-clone-991c0.appspot.com",
            messagingSenderId: "1082900957395",
            appId: "1:1082900957395:web:93cee2c0b57775d0"
        };
        // Initialize Firebase
                ...
                if (snapshot.exists()) {
                    ...
                    snapshot.forEach(item => {
                        ...
                        if (data.type === 'folder') {
                            ...
                            this.removeFileStorage(ref + '/' + name, data.name).then(() => {
                    ...
                    this.getFirebaseRef('Meus arquivos').child(key).remove();
            ...
            promises.push(new Promise((resolve, reject) => {            
                ...
                if (file.type === 'folder') {
                    ...
                    this.removeFolderTask(this.currentFolder.join('/'), file.name, key).then(() => {
                        ...
                        //fields: {key} are params to delete when 'click' on btnDelete
                        resolve({
                            fields:{key}
                        });
                    ...
                    this.removeFileStorage(this.currentFolder.join('/'), file.name).then(() => {
        ...
        this.inputFilesEl.addEventListener('change', event => {
            ...
            this.upLoadTask(event.target.files).then(responses => {
                ...
                responses.forEach(resp => {
                    ...
                    //To add on firebase
                    //push() is to add on firebase and it works like an array
                    //set() indicate the data that needs to be save... use [] because of the "-" on names 
                    //when using a local storage -> this.getFirebaseRef().push().set(resp.files['input-file'])

                    //using firebase storage
                    resp.ref.getDownloadURL().then(data => {

                        this.getFirebaseRef().push().set({
                            md5hash: resp.md5Hash,
                            name: resp.name,
    ...
    //To get a reference for firebase
    getFirebaseRef(path){
    ...
    //to show or not the progress bar
    modalShow(show = true){
    ...
    ajax(url, method = 'GET', formData = new formData(), onprogress = function(){}, onloadstart = function(){} ){
        ...
        return new Promise((resolve, reject) => {
            ...
            ajax.onload = event =>{
                ...
                //To receive the progress by bites from the upload
    ...
    upLoadTask(files){
        ...
        [...files].forEach(file => {
            ...
            promises.push(new Promise((resolve, reject) => {
                ...
                //to save in firebase storage
                let fileRef = firebase.storage().ref(this.currentFolder.join('/')).child(file.name);

                //put() to upload the file
                let task = fileRef.put(file);

                //on('state_changed', progress, error, resolve) to listen the changes
            ...
            /*To save in a local server
            //FormData is to read the file 
            //1º param is the post name that the server will receive and the 2° is the file
        ...
        //Promise.all() do the Promise for each item inside the array
        return Promise.all(promises);

    }

    //To update the progress bar and the time left
    ...
    //To read the data from firebase
    readFiles(){

        this.lastFolder = this.currentFolder.join('/');

        //on() keep "listening" the db to see if something has changed in db
        //value is the event that happens when something change in firebase
        //snapshot is like a photo of the event that return 
        this.getFirebaseRef().on('value', snapshot => {
            ...
            snapshot.forEach(snapshotItem => {
                ...
                //val() is the method to get the data
        ...
        li.addEventListener('click', e => {
            ...
            if(e.shiftKey) {
                ...
                if (firstLi) {
                    ...
                    let childUl = li.parentElement.childNodes;

                    childUl.forEach((el, index) => {
                    ...
                    //sort() let array in growing order
                    let indexSE = [indexStart, indexEnd].sort();

                    childUl.forEach((el, index) => {

                        if (index >= indexSE[0] && index <= indexSE[1]){
                    ...
                    //To stop the execution before "li.classList.toggle('selected');"
                    return true;
